...
type ConfirmationUpdateResult struct {
	Confirmations            []*ethrpc.MinimalBlockInfo `json:"confirmations,omitempty" ffstruct:"ConfirmationUpdateResult"`     // the confirmation list
	Rebuilt                  bool                       `json:"rebuilt,omitempty" ffstruct:"ConfirmationUpdateResult"`           // when true, it means the existing confirmations contained invalid blocks, the new confirmations are rebuilt from scratch
	NewFork                  bool                       `json:"newFork,omitempty" ffstruct:"ConfirmationUpdateResult"`           // when true, it means a new fork was detected based on the existing confirmations
	Confirmed                bool                       `json:"confirmed,omitempty" ffstruct:"ConfirmationUpdateResult"`         // when true, it means the confirmation list is complete and the transaction is confirmed
	TargetConfirmationCount  uint64                     `json:"targetConfirmationCount" ffstruct:"ConfirmationUpdateResult"`     // the target number of confirmations for this reconcile request
	CurrentConfirmationCount uint64                     `json:"currentConfirmationCount" ffstruct:"ConfirmationUpdateResult"`    // the current number of confirmations for this reconcile request
	TxnBlockTimestamp        *ethtypes.HexUint64        `json:"txnBlockTimestamp,omitempty" ffstruct:"ConfirmationUpdateResult"` // the on-chain timestamp of the block the transaction was included in - not a confirmation-finality guarantee, this can change if the chain forks before Confirmed becomes true. Only populated in "full" chain tracking mode, since "light" mode never fetches a block
}
