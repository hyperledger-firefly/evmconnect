...
var (
	...
	_ = ffc("config.connector.events.filterPollingMode", "How new blocks, and events in the steady state event loop once caught up with the head of the chain, are polled for. 'server' establishes node-side filters (eth_newBlockFilter for new blocks, eth_newFilter for events) and polls them with eth_getFilterChanges. 'client' polls the latest block with eth_getBlockByNumber and uses stateless eth_getLogs range queries, with the connector tracking its own poll positions, avoiding node-side filter state entirely. With chainTrackingMode 'light' new blocks are only ever detected by polling eth_blockNumber, in either mode. In 'client' mode with chainTrackingMode 'light', the checkpointBlockGap window of blocks behind the head is re-scanned on each polling interval to detect events moved by a re-org, with block-hash de-duplication of events already delivered. Fixed to 'client' in light chain tracking mode", "server,client")
