...
var (
	...
	_ = ffc("config.connector.receiptCache.enabled", "When true, transaction receipts fetched during canonical chain build are cached in memory and served for receipt queries.", i18n.BooleanType)
	_ = ffc("config.connector.receiptCache.size", "Maximum number of transaction receipts to hold in the receipt cache", i18n.IntType)
