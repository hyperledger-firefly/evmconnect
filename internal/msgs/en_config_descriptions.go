...
var (
	...
	_ = ffc("config.connector.events.headTrackingMode", "How the event stream tracks the head of the chain in the steady state event loop, once caught up. 'server-filter' establishes a node-side filter with eth_newFilter and polls it with eth_getFilterChanges. 'client-only' uses stateless eth_getLogs range queries with the connector tracking its own poll position, avoiding node-side filter state entirely", "server-filter,client-only")
