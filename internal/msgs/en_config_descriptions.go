...
var (
	...
	_ = ffc("config.connector.events.filterPollingMode", "How the steady state event loop polls for new events once caught up with the head of the chain. 'filter' establishes a node-side filter with eth_newFilter and polls it with eth_getFilterChanges. 'getLogs' uses stateless eth_getLogs range queries with the connector tracking its own poll position, avoiding node-side filter state entirely", "filter,getLogs")
